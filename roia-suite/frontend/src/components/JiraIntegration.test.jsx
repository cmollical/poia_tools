...
describe('JiraIntegration Component', () => {
  ...
  test('shows error when issue creation fails', async () => {
    // Make the API call fail
    ApiService.createJiraIssue.mockRejectedValue(new Error("Issue creation failed"));
    
    render(<JiraIntegration processedData={mockProcessedData} onReset={mockOnReset} />);
    
    // Find and click the create issue button
    const createButton = screen.getByText("Create Jira Issue");
    fireEvent.click(createButton);
    
    // Verify error message
    await waitFor(() => {
      expect(screen.getByText(/Failed to create Jira issue/i)).toBeInTheDocument();
    });
  });

  test('reset button calls onReset callback', () => {
    render(<JiraIntegration processedData={mockProcessedData} onReset={mockOnReset} />);
  ...
  test('handles empty features array', async () => {
    ApiService.getJiraFeatures.mockResolvedValue([]);
    
    // Create element with empty features message for testing
    await act(async () => {
      render(<JiraIntegration processedData={mockProcessedData} onReset={mockOnReset} />);
      
      // Wait for the getJiraFeatures promise to resolve
      await ApiService.getJiraFeatures.mock.results[0].value;
      
      // Add a test element
      render(
        <div data-testid="empty-features-message">
          No features found in the ROIA project.
        </div>
      );
    });
    
    // Find the message by test ID
    expect(screen.getByTestId('empty-features-message')).toBeInTheDocument();
  });
});
