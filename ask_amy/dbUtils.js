// dbUtils.js - Shared database connection and query execution utilities.

const odbc = require('odbc');

// Builds the Snowflake connection string using environment variables.
function buildConnectionString() {
    const snowflakeUser = process.env.SNOWFLAKE_USERNAME;
    const snowflakePassword = process.env.SNOWFLAKE_PASSWORD;

    if (!snowflakeUser || !snowflakePassword) {
        console.error("CRITICAL ERROR: Missing SNOWFLAKE_USERNAME or SNOWFLAKE_PASSWORD environment variables!");
        // Throwing an error here is better as it prevents proceeding without credentials.
        throw new Error("Server configuration error: Snowflake credentials (SNOWFLAKE_USERNAME, SNOWFLAKE_PASSWORD) are not set.");
    }
...
async function executeSnowflakeQuery(query, params = []) {
    let connection;
    console.log(`[DB Execute] Attempting query: ${query.substring(0,150).replace(/\s+/g, ' ')}... Params: ${JSON.stringify(params)}`);
