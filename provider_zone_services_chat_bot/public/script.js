document.addEventListener('DOMContentLoaded', () => {
    ...
    async function checkAdminAccess() {
        ...
        if (!currentUser) {
            document.getElementById('admin-button-container').classList.add('hidden');
            return;
        }
        
        try {
            // Attempt to access the admin API endpoint
            const response = await fetch('/api/admins', {
                headers: getAuthHeader()
            });
            
            // If request is successful, the user is an admin
            if (response.ok) {
    ...
    if (sessionStorage.getItem('jwt_token')) {
        loadFiles();
        checkAdminAccess(); // This is now async but we don't need to block on it
        showChat();
    } else {
        showLogin();
    }
    ...
    document.getElementById('admin-button')?.addEventListener('click', (e) => {
        e.preventDefault();
        // Use fetchWithAuth to get the admin page with proper authentication
    ...
    loginBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            // Store the username to check for admin access
            sessionStorage.setItem('current_user', user);
            // Also set a client-side cookie for redundancy
            document.cookie = `jwt_token=${data.token}; path=/; max-age=${8 * 60 * 60}`;
